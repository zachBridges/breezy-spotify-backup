...
SPOTIFY_REFRESH_TOKEN=$(echo "$RESPONSE" | jq -r '.refresh_token')
grep -qxF "export SPOTIFY_ACCESS_TOKEN=${SPOTIFY_ACCESS_TOKEN}" .envrc || printf "\nexport SPOTIFY_ACCESS_TOKEN=${SPOTIFY_ACCESS_TOKEN}" >> .envrc